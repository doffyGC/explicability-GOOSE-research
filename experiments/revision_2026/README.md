...
| `cross_run_report.py` | Checklist D.5's consolidation: every card-D run in one table, on one pool, per class. The card's evidence had grown to seven documents (`pr_curves_d1`, `pr_curves_d1_split`, `pr_curves_d2`, `pr_curves_d2_rules`, `pr_curves_d3_v2`, `pr_curves_d4`, four `run_bootstrap.*`), none showing more than one card at a time and none checked against the others. Reads **only** each run's `grouped_validation_report.json`, so 24 runs consolidate in under a second: a fold's `per_class` block carries recall, precision and support, and the folds partition the evaluated rows exactly once, so `TP = recall x support`, `FP = TP/precision - TP` summed over folds recovers the pooled counts. Metrics are **pooled** (counts summed, metric computed once), never averaged over folds — the folds partition the runs rather than replicate them, so a fold mean would weight a fold holding one `SAG.DB` run the same as one holding four. The reconstruction is verified rather than trusted: `TP` must land on an integer (observed deviation 0.0000 across all 24 runs), `--confusion` rebuilds the real matrix from a run's predictions and checks the pooled metrics against `check_prediction_integrity.metrics_from_confusion` at 1e-9 (five runs verified), and the tests reproduce `run_bootstrap.champion_v2.md`'s published per-class numbers and macro F1 (0.7310) — computed the expensive way from 11M rows — to four decimals. Refuses what cannot share a table: two dataset hashes, two split files, two protocols, two fold counts, two evaluated-row counts, or a technical smoke without `--allow-smoke`. `--pr-curves` merges `ANY_ATTACK` AP from any number of curve reports so the table carries the ranking axis beside the argmax one (§11), and a curve file built on another pool is fatal rather than skipped. A rule run's non-designated attack classes are rendered `n/a`, not zero: a rule has one score and cannot name a family, so those cells are the prevalence floor by construction (`ablations_baselines.md` §16). Computes **no interval of its own** and says so in its own output — the paired comparisons stay in `grouped_pr_curves.py` and `bootstrap_run_intervals.py`, which own the resampling. | D.5 |
| `test_cross_run_report.py` | Pins the reconstruction against `check_prediction_integrity.metrics_from_confusion` by round-tripping synthetic confusion matrices into per-fold report blocks and back, on deliberately lopsided folds where the pooled result and the fold mean differ; reproduces the published champion numbers; and covers the refusals — mixed pools, mixed split files, mixed protocols, mixed fold counts, mixed row counts, technical smokes, a curve file from another pool, two curve files disagreeing about one run's AP, and a rule that designated different classes in different folds. 32 tests. | D.5 |
| `run_grouped_shap.py` | Card F.3/F.4: SHAP on **held-out grouped data**, with the background distribution taken from the same fold's **train** partition. Closes the last of the four defects that motivated this revision — the frozen baseline's `explainability/shap_analysis.py` explains the last CV fold's validation set, which under message-level `StratifiedKFold` holds messages from the runs the model was fitted on, so every importance in the submitted paper was measured partly on training data. No run persists a fitted estimator, so this card refits per fold; the refit is reconstructed exactly as `run_folds` builds it (same persisted fold, `seed + fold_index`, feature set, library defaults and `n_jobs`) and **must reproduce the reference run's `grouped_predictions.csv` row for row** — a single mismatched row is fatal, because SHAP on a different model than the published one explains nothing about the paper. Mismatched model, seed or thread count are refused before any fitting starts, and a capped technical smoke with verification on is refused with its reason rather than failing later for the wrong one. `feature_perturbation="interventional"` is asserted after construction, not assumed: `tree_path_dependent` would silently ignore the background and answer a different question. Values are kept **per class**, never summed across classes, and reported **per fold** — the spread is F.4's deliverable, shown rather than averaged, and explicitly not an interval (five folds are one partition of the same 265 runs). The explained rows are (`split_group`, `class`)-stratified so every test run stays represented; the background is stratified **by class only**, because group-stratifying it imposes a floor of one row per stratum — it returned 411 rows for a cap of 100 on a smoke, and interventional SHAP costs O(explained × background). `--limit-folds` prices the card on the real pool before committing to five folds and marks its output `partial_probe`. | F.3, F.4 |
| `test_grouped_shap.py` | The load-bearing test is end to end: produce a reference run with `run_grouped_validation.py`, then explain it **with verification on**, so a pass means the fold reconstruction, the seeding and the estimator construction are equivalent to the runner's — established by outcome rather than by reading two code paths side by side. Also covers the verification's own failure modes (one disagreeing row, a different row count, different rows covered, a fold missing from the reference, and that it compares by `row_index` rather than by position), the refusals (wrong pool, wrong split file, a technical-smoke reference, mismatched model/seed/`n_jobs`, a capped smoke with verification on), the two samplers (every group and class survives the explained-row cap; the background returns the size asked for rather than a stratum floor, loses no class, and repeats no row), and that `--skip-verification` marks the output UNVERIFIED in both artifacts. 33 tests. | F.3, F.4 |
